...
class GmpGetReportVulnsTestMixin:
    def test_get_report_vulns_without_id(self):
        with self.assertRaises(RequiredArgument):
            self.gmp.get_report_vulns(None)

        with self.assertRaises(RequiredArgument):
            self.gmp.get_report_vulns("")

    def test_get_report_vulns_with_filter_string(self):
        self.gmp.get_report_vulns(report_id="r1", filter_string="name=foo")
    ...
    def test_get_report_vulns_with_filter_id(self):
        self.gmp.get_report_vulns(report_id="r1", filter_id="f1")
    ...
    def test_get_report_vulns_with_ignore_pagination(self):
        self.gmp.get_report_vulns(report_id="r1", ignore_pagination=True)

        self.connection.send.has_been_called_with(
            b'<get_report_vulns report_id="r1" ignore_pagination="1" details="1"/>'
        )

        self.gmp.get_report_vulns(report_id="r1", ignore_pagination=False)
    ...
    def test_get_report_vulns_with_details(self):
        self.gmp.get_report_vulns(report_id="r1", details=True)

        self.connection.send.has_been_called_with(
            b'<get_report_vulns report_id="r1" details="1"/>'
        )

        self.gmp.get_report_vulns(report_id="r1", details=False)
